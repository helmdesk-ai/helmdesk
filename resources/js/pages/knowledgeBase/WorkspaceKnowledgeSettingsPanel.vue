<!--
  工作区知识库检索配置面板，内嵌在知识库列表右侧区域，供管理员统一配置嵌入模型、分段策略、
...
import type {
  ...
  WorkspaceKnowledgeSettingsData,
} from '@/types/generated';
import { Form } from '@inertiajs/vue3';
import { computed, ref, watch } from 'vue';

const props = defineProps<{
  settings: WorkspaceKnowledgeSettingsData;
  ...
  <div class="mx-auto w-full max-w-none space-y-6">
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div class="space-y-1">
          <Label for="workspace-vector-enabled">{{ t('标准索引') }}</Label>
      ...
      <FormField
        ...
        :label="t('嵌入模型')"
        label-for="workspace-embedding-model"
        :error="errors.embedding_model_id"
      >
        <Select v-model="selectedEmbeddingModelId">
          <SelectTrigger id="workspace-embedding-model" class="mt-1 w-full">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <FormField
        v-if="vectorIndexEnabled"
        :label="t('向量维度')"
        label-for="workspace-embedding-dimension"
        :error="errors.embedding_dimension"
      >
        <Input
          id="workspace-embedding-dimension"
      ...
      <FormField
        v-if="vectorIndexEnabled"
        :label="t('分段方式')"
        label-for="workspace-chunking-strategy"
      >
        <Select v-model="selectedChunkingStrategy">
          <SelectTrigger id="workspace-chunking-strategy" class="mt-1 w-full">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem
      ...
      <FormField
        v-if="vectorIndexEnabled"
        :label="t('单段最大 token')"
        label-for="workspace-chunk-max"
        :error="errors.chunk_max_tokens"
      >
        <Input
          id="workspace-chunk-max"
      ...
      <FormField
        v-if="vectorIndexEnabled"
        :label="t('相邻段重叠 token')"
        label-for="workspace-chunk-overlap"
        :error="errors.chunk_overlap_tokens"
      >
        <Input
          id="workspace-chunk-overlap"
      ...
      <div class="flex items-start justify-between gap-4">
        <div class="space-y-1">
          <Label for="workspace-raptor-enabled">{{ t('深度索引') }}</Label>
          <p class="text-sm text-muted-foreground">
      ...
      <FormField
        ...
        :label="t('摘要模型')"
        label-for="workspace-summary-model"
        :error="errors.summary_model_id"
      >
        <Select v-model="selectedSummaryModelId">
          <SelectTrigger id="workspace-summary-model" class="mt-1 w-full">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <FormField
        :label="t('重排序模型（可选）')"
        label-for="workspace-rerank-model"
        :error="errors.rerank_model_id"
      >
        <Select v-model="selectedRerankModelId">
          <SelectTrigger id="workspace-rerank-model" class="mt-1 w-full">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>

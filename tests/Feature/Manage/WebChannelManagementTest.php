...
test('所有者可以查看网页频道列表和详情页面', function () {
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.channels.web.index'))
        ->assertOk()
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.channels.web.create'))
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.channels.web.show', ['channel' => $channel->id]))
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
...
test('所有者可以创建活跃频道并带名称和接待方案版本', function () {
    ...
    $response = $this->actingAs($this->user)
        ->from(route('workspace.manage.channels.web.index'))
        ->post(route('workspace.manage.channels.web.store'), [
            'name' => '帮助中心',
    ...
    $response->assertRedirect(route('workspace.manage.channels.web.show', ['channel' => $channel->id,
    ]));
...
test('创建频道需要名称', function () {
    ...
    $this->actingAs($this->user)
        ->from(route('workspace.manage.channels.web.create'))
        ->post(route('workspace.manage.channels.web.store'), [
        ])
...
test('所有者可以保存基础频道信息', function () {
    ...
    $this->actingAs($this->user)
        ...
        ->put(route('workspace.manage.channels.web.basic.update', ['channel' => $channel->id]), [
            'name' => '帮助中心',
...
test('所有者可以保存小部件入口配置', function () {
    ...
    $this->actingAs($this->user)
        ->put(route('workspace.manage.channels.web.widget.update', ['channel' => $channel->id]), [
...
test('接入方式表单仅更新嵌入域名白名单且不影响入口配置', function () {
    ...
    $this->actingAs($this->user)
        ->put(route('workspace.manage.channels.web.access.update', ['channel' => $channel->id]), [
...
test('所有者可以保存访客界面配置并同步到两个入口', function () {
    ...
    $detailUrl = route('workspace.manage.channels.web.show', ['channel' => $channel->id,
        'tab' => 'visitor-interface',
    ]);

    $this->actingAs($this->user)
        ->from($detailUrl)
        ->put(route('workspace.manage.channels.web.visitor-interface.update', ['channel' => $channel->id]), [
            'site_name' => '帮助中心',
            'subtitle' => '产品咨询与售后支持',
            'header_enabled' => '1',
...
test('所有者可以软删除频道', function () {
    ...
    $this->actingAs($this->user)
        ->delete(route('workspace.manage.channels.web.destroy', ['channel' => $channel->id]))
        ->assertRedirect(route('workspace.manage.channels.web.index'));
...
test('所有者可以查看频道回收站和恢复频道', function () {
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.channels.web.trash'))
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
    ...
    $this->actingAs($this->user)
        ->from(route('workspace.manage.channels.web.trash'))
        ->put(route('workspace.manage.channels.web.restore', ['channel' => $channel->id]))
        ->assertRedirect();
...
test('单租户下管理员可以访问或删除任意频道', function () {
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.channels.web.show', ['channel' => $otherChannel->id]))
        ->assertOk();

    $this->actingAs($this->user)
        ->delete(route('workspace.manage.channels.web.destroy', ['channel' => $otherChannel->id]))
        ->assertRedirect();

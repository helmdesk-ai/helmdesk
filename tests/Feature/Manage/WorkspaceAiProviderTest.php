...
test('访客用户不能访问工作区AI提供商设置', function () {
    $this->get(route('workspace.manage.ai.providers.index'))
...
test('非所有者工作区成员不能访问AI提供商设置', function () {
    ...
    $this->actingAs($admin)
        ->get(route('workspace.manage.ai.providers.index'))
        ->assertForbidden();
...
test('所有者可以从内置品牌一步创建供应商并带内置模型', function () {
    $this->actingAs($this->user)
        ->post(route('workspace.manage.ai.providers.store'), [
            'brand' => 'deepseek',
            'name' => '主要 DeepSeek',
            'configuration' => [
                'key' => 'sk-deepseek',
...
test('同一品牌可以重复添加多个供应商', function () {
    ...
    $this->actingAs($this->user)
        ->post(route('workspace.manage.ai.providers.store'), $payload)
        ->assertRedirect();
...
test('自定义品牌必须填写名称', function () {
    $this->actingAs($this->user)
        ->from(route('workspace.manage.ai.providers.index'))
        ->post(route('workspace.manage.ai.providers.store'), [
            'brand' => 'custom-openai',
            'configuration' => [
                'key' => 'sk-custom',
...
test('创建供应商时缺少必填凭据报校验错误', function () {
    $this->actingAs($this->user)
        ...
        ->assertRedirect(route('workspace.manage.ai.providers.index'))
...
test('所有者可以配置提供商凭证', function () {
    ...
    $this->actingAs($this->user)
        ->put(route('workspace.manage.ai.providers.update', ['provider' => $provider->slug,
        ]), [
            'configuration' => [
...
test('所有者可以删除任意供应商', function () {
    ...
    $this->actingAs($this->user)
        ->delete(route('workspace.manage.ai.providers.destroy', ['provider' => $provider->slug,
        ]))
        ->assertRedirect();
...
test('所有者可以添加自定义模型到提供商在其工作区', function () {
    ...
    $this->actingAs($this->user)
        ->post(route('workspace.manage.ai.models.store', ['provider' => $provider->slug,
        ]), [
...
test('所有者可以切换模型活跃状态', function () {
    ...
    $this->actingAs($this->user)
        ->put(route('workspace.manage.ai.models.toggle', ['provider' => $provider->slug,
            'model' => $model->id,
        ]))
        ->assertRedirect();
...
test('所有者可以删除自定义模型', function () {
    ...
    $this->actingAs($this->user)
        ->delete(route('workspace.manage.ai.models.destroy', ['provider' => $provider->slug,
            'model' => $model->id,
        ]))
        ->assertRedirect();
...
test('检查连接返回no_model当提供商没有活跃LLM', function () {
    ...
    $this->actingAs($this->user)
        ->postJson(route('workspace.manage.ai.providers.check', ['provider' => $provider->slug,
        ]), [
            'configuration' => [
                'key' => 'azure-test-key',

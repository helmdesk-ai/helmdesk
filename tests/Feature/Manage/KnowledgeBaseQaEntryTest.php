...
test('所有者可以创建带相似问法和多答案的问答条目', function () {
    ...
    $this->actingAs($this->user)
        ->post(
            route('workspace.manage.knowledge-bases.qa-entries.store', [
                'slug' => $this->workspaceSlug(),
                'knowledgeBase' => $this->qaKnowledgeBase->id,
            ]),
            [
                'question' => '如何申请退款？',
    ...
    expect($entry->workspace_id)->toBe((string) $this->workspace->id)
        ->and($entry->knowledge_base_id)->toBe((string) $this->qaKnowledgeBase->id)
...
test('问答条目只能添加到问答知识库', function () {
    ...
    $this->actingAs($this->user)
        ...
        ->post(
            route('workspace.manage.knowledge-bases.qa-entries.store', [
                'slug' => $this->workspaceSlug(),
                'knowledgeBase' => $standardKnowledgeBase->id,
            ]),
            [
                'question' => '如何申请退款？',
                'answers' => ['请在订单详情页提交退款申请。'],
...
test('编辑问答条目会替换相似问法和答案', function () {
    ...
    $this->actingAs($this->user)
        ->put(
            route('workspace.manage.knowledge-bases.qa-entries.update', [
                'slug' => $this->workspaceSlug(),
                'knowledgeBase' => $this->qaKnowledgeBase->id,
                'entry' => $entry->id,
            ]),
...
test('问答条目可以移动分组', function () {
    ...
    $this->actingAs($this->user)
        ->put(
            route('workspace.manage.knowledge-bases.qa-entries.move', [
                'slug' => $this->workspaceSlug(),
                'knowledgeBase' => $this->qaKnowledgeBase->id,
                'entry' => $entry->id,
            ]),
...
test('删除问答条目会同步删除相似问法和答案', function () {
    ...
    $this->actingAs($this->user)
        ->delete(
            route('workspace.manage.knowledge-bases.qa-entries.destroy', [
                'slug' => $this->workspaceSlug(),
                'knowledgeBase' => $this->qaKnowledgeBase->id,
                'entry' => $entry->id,
            ])

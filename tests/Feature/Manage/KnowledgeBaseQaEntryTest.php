...
test('所有者可以创建带相似问法和多答案的问答条目', function () {
    ...
    $this->actingAs($this->user)
        ->post(
            route('workspace.manage.knowledge-bases.qa-entries.store', ['knowledgeBase' => $this->qaKnowledgeBase->id,
            ]),
            [
                'question' => '如何申请退款？',
...
test('问答条目只能添加到问答知识库', function () {
    ...
    $this->actingAs($this->user)
        ...
        ->post(
            route('workspace.manage.knowledge-bases.qa-entries.store', ['knowledgeBase' => $standardKnowledgeBase->id,
            ]),
            [
                'question' => '如何申请退款？',
                'answers' => ['请在订单详情页提交退款申请。'],
...
test('编辑问答条目会替换相似问法和答案', function () {
    ...
    $this->actingAs($this->user)
        ->put(
            route('workspace.manage.knowledge-bases.qa-entries.update', ['knowledgeBase' => $this->qaKnowledgeBase->id,
                'entry' => $entry->id,
            ]),
            [
                'question' => '新问题',
...
test('问答条目可以移动分组', function () {
    ...
    $this->actingAs($this->user)
        ->put(
            route('workspace.manage.knowledge-bases.qa-entries.move', ['knowledgeBase' => $this->qaKnowledgeBase->id,
                'entry' => $entry->id,
            ]),
            ['group_id' => $group->id]
        )
...
test('包含问答条目的分组不能删除', function () {
    ...
    $this->actingAs($this->user)
        ->from(route('workspace.manage.knowledge-bases.index'))
        ->delete(
            route('workspace.manage.knowledge-bases.groups.destroy', ['knowledgeBase' => $this->qaKnowledgeBase->id,
                'group' => $group->id,
            ])
        )
...
test('问答知识库列表返回问答条目并可按相似问法和答案搜索', function () {
    ...
    $this->actingAs($this->user)
        ->get(
            route('workspace.manage.knowledge-bases.index', ['kb' => $this->qaKnowledgeBase->id,
                'search' => '退款',
            ])
        )
...
test('删除问答条目会同步删除相似问法和答案', function () {
    ...
    $this->actingAs($this->user)
        ->delete(
            route('workspace.manage.knowledge-bases.qa-entries.destroy', ['knowledgeBase' => $this->qaKnowledgeBase->id,
                'entry' => $entry->id,
            ])
        )
        ->assertRedirect();

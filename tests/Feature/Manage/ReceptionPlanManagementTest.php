...
test('所有者可以查看接待方案列表页', function () {
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.reception.plans.index'))
        ->assertOk()
...
test('创建接待方案时语气风格非法值会被校验拒绝', function () {
    ...
    $this->actingAs($this->user)
        ->post(route('workspace.manage.reception.plans.store'), [
            'name' => '非法语气方案',
            'description' => null,
...
test('所有者可以创建接待方案草稿', function () {
    ...
    $response->assertRedirect(route('workspace.manage.reception.plans.show', ['plan' => $plan->id,
    ]));
...
test('所有者可以更新接待方案草稿', function () {
    ...
    $this->actingAs($this->user)
        ->from(route('workspace.manage.reception.plans.index', ['plan' => $plan->id,
        ]))
...
test('保存接待方案即生成新版本快照且配置无变化时不新增版本', function () {
    ...
    // 首次保存：生成 v1
    $this->actingAs($this->user)
        ->put(route('workspace.manage.reception.plans.update', ['plan' => $plan->id]), $payload)
        ->assertRedirect();
...
test('所有者可以删除接待方案当没有版本引用时', function () {
    ...
    $this->actingAs($this->user)
        ->delete(route('workspace.manage.reception.plans.destroy', ['plan' => $plan->id]))
        ->assertRedirect(route('workspace.manage.reception.plans.index'));
...
test('所有者可以查看接待方案回收站并恢复方案', function () {
    ...
    $this->actingAs($this->user)
        ...
        ->put(route('workspace.manage.reception.plans.restore', ['plan' => $plan->id,
        ]))
...
test('单租户下管理员可以访问任意方案详情', function () {
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.reception.plans.show', ['plan' => $localPlan->id,
        ]))
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
            ->component('reception/plans/Detail')
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.reception.plans.show', ['plan' => $otherPlan->id,
        ]))
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
            ->component('reception/plans/Detail')
    ...
    $this->actingAs($this->user)
        ->delete(route('workspace.manage.reception.plans.destroy', ['plan' => $otherPlan->id]))
        ->assertRedirect();

...
test('所有者可以查看接待方案列表页', function () {
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.reception.plans.index', ['slug' => $this->workspaceSlug()]))
        ->assertOk()
...
test('接待方案管理页下发模型与枚举选项', function () {
    $provider = createReceptionTestProvider();
    $model = createReceptionTestModel($provider);
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.reception.plans.show', ['slug' => $this->workspaceSlug(), 'plan' => $plan->id]))
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
            ->component('reception/plans/Detail')
            ->has('llm_model_options', 1)
...
test('创建接待方案时语气风格非法值会被校验拒绝', function () {
    ...
    $this->actingAs($this->user)
        ->post(route('workspace.manage.reception.plans.store', ['slug' => $this->workspaceSlug()]), [
            'name' => '非法语气方案',
            'description' => null,
...
test('创建接待方案即生成初始版本快照', function () {
    ...
    $plan = ReceptionPlan::query()->where('workspace_id', $this->workspace->id)->firstOrFail();
...
test('所有者可以创建接待方案草稿', function () {
    ...
    $backup = createReceptionTestModel($provider, [
        'name' => 'Backup Model',
        'model_id' => 'backup-model',
        'sort_order' => 2,
    ]);
    $taskBackup = createReceptionTestModel($provider, [
        'name' => 'Task Backup Model',
        'model_id' => 'task-backup-model',
        'sort_order' => 3,
    ]);
    ...
    $response->assertRedirect(route('workspace.manage.reception.plans.show', [
    ...
    expect($plan->name)->toBe('售前接待方案')
        ...
        ->and($plan->reception_config['default_model']['ai_model_id'])->toBe($model->id)
        ->and($plan->reception_config['model_candidates'])->toBe([
            ['ai_model_id' => $model->id, 'priority' => 0],
            ['ai_model_id' => $backup->id, 'priority' => 1],
        ])
        ->and($plan->task_config['default_model']['ai_model_id'])->toBe($task->id)
        ->and($plan->task_config['model_candidates'])->toBe([
            ['ai_model_id' => $task->id, 'priority' => 0],
            ['ai_model_id' => $taskBackup->id, 'priority' => 1],
        ])
...
test('创建接待方案时未指定模型校验失败', function () {
    $provider = createReceptionTestProvider();
    $model = createReceptionTestModel($provider);
...
test('所有者可以更新接待方案草稿', function () {
    ...
    $backupModel = createReceptionTestModel($provider, [
        'name' => 'Backup Model',
        'model_id' => 'backup-model',
        'sort_order' => 2,
    ]);
    ...
    $taskBackupModel = createReceptionTestModel($provider, [
        'name' => 'Task Backup Model',
        'model_id' => 'task-backup-model',
        'sort_order' => 4,
    ]);
    ...
    $this->actingAs($this->user)
        ->from(route('workspace.manage.reception.plans.index', [
            'slug' => $this->workspaceSlug(),
            'plan' => $plan->id,
        ]))
        ->put(route('workspace.manage.reception.plans.update', ['slug' => $this->workspaceSlug(), 'plan' => $plan->id]), [
            'name' => '更新后的名称',
            'description' => '更新后说明',
            'persona_display_name' => '小 B',
            'persona_tone' => 'concise',
            'global_instructions' => '更新后的指引',
            'reception_ai_model_id' => $updatedModel->id,
            'reception_model_candidates' => [
                ['ai_model_id' => $backupModel->id, 'priority' => 1],
            ],
            'task_ai_model_id' => $taskModel->id,
            'task_model_candidates' => [
                ['ai_model_id' => $taskBackupModel->id, 'priority' => 1],
            ],
            'service_scenarios' => [],
            'strategy_config' => $strategyConfig,
    ...
    expect($plan->name)->toBe('更新后的名称')
        ...
        ->and($plan->reception_config['default_model']['ai_model_id'])->toBe($updatedModel->id)
        ->and($plan->reception_config['model_candidates'])->toBe([
            ['ai_model_id' => $updatedModel->id, 'priority' => 0],
            ['ai_model_id' => $backupModel->id, 'priority' => 1],
        ])
        ->and($plan->task_config['default_model']['ai_model_id'])->toBe($taskModel->id)
        ->and($plan->task_config['model_candidates'])->toBe([
...
test('保存接待方案即生成新版本快照且配置无变化时不新增版本', function () {
    ...
    // 首次保存：生成 v1
    $this->actingAs($this->user)
        ->put(route('workspace.manage.reception.plans.update', ['slug' => $this->workspaceSlug(), 'plan' => $plan->id]), $payload)
        ->assertRedirect();
...
test('更新接待方案时备用模型不能重复主模型', function () {
    $provider = createReceptionTestProvider();
    $model = createReceptionTestModel($provider);
    ...
    $this->actingAs($this->user)
        ...
        ->put(route('workspace.manage.reception.plans.update', ['slug' => $this->workspaceSlug(), 'plan' => $plan->id]), [
            ...
            'reception_ai_model_id' => $model->id,
            'reception_model_candidates' => [
                ['ai_model_id' => $model->id, 'priority' => 1],
            ],
            'task_ai_model_id' => $model->id,
            'service_scenarios' => [],
            'strategy_config' => receptionPlanStrategyPayload(),
...
test('所有者可以删除接待方案当没有版本引用时', function () {
    ...
    $this->actingAs($this->user)
        ->delete(route('workspace.manage.reception.plans.destroy', ['slug' => $this->workspaceSlug(), 'plan' => $plan->id]))
        ->assertRedirect(route('workspace.manage.reception.plans.index', ['slug' => $this->workspaceSlug()]));
...
test('所有者可以查看接待方案回收站并恢复方案', function () {
    ...
    $this->actingAs($this->user)
        ->get(route('workspace.manage.reception.plans.trash', [
            'slug' => $this->workspaceSlug(),
        ]))
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('trashed_plan_list.0.reception_model.label', 'Test Provider / Test Model (gpt-4o)')
            ->where('trashed_plan_list.0.reception_model_status.is_valid', true)
            ->where('trashed_plan_list.0.task_model.label', 'Test Provider / Task Model (task-model)')
            ->where('trashed_plan_list.0.task_model_status.label', 'Test Provider / Task Model (task-model)')
            ->where('trashed_plan_list_pagination.total', 1)
        );

    $this->actingAs($this->user)
        ->from(route('workspace.manage.reception.plans.trash', [
            'slug' => $this->workspaceSlug(),
        ]))
        ->put(route('workspace.manage.reception.plans.restore', [
            'slug' => $this->workspaceSlug(),
            'plan' => $plan->id,
        ]))
        ->assertRedirect(route('workspace.manage.reception.plans.trash', [
            'slug' => $this->workspaceSlug(),
        ]));
...
test('当 PlanVersion 仍被会话引用时阻止删除', function () {
    ...
    $this->actingAs($this->user)
        ->from(route('workspace.manage.reception.plans.index', ['slug' => $this->workspaceSlug()]))
...
test('保存接待方案生成的版本快照含编译后的运行时配置', function () {
    ...
    $this->actingAs($this->user)
        ->put(route('workspace.manage.reception.plans.update', ['slug' => $this->workspaceSlug(), 'plan' => $plan->id]), [
            ...
            'global_instructions' => '保持简洁',
            'reception_ai_model_id' => $model->id,
            'reception_model_candidates' => [['ai_model_id' => $backup->id, 'priority' => 1]],
            'task_ai_model_id' => $task->id,
            'task_model_candidates' => [['ai_model_id' => $taskBackup->id, 'priority' => 1]],
            'service_scenarios' => [],
            'strategy_config' => $strategyConfig,
            'auto_messages_config' => receptionPlanAutoMessagesPayload(),
            'translation_config' => receptionPlanTranslationPayload(),
    ...
    expect($version->version_number)->toBe(1)
        ...
        ->and($version->compiled_config['reception_config']['default_model']['ai_model_id'])->toBe($model->id)
        ->and($version->compiled_config['reception_config']['model_candidates'])->toBe([
            ['ai_model_id' => $model->id, 'priority' => 0],
            ['ai_model_id' => $backup->id, 'priority' => 1],
        ])
        ->and($version->compiled_config['task_config']['default_model']['ai_model_id'])->toBe($task->id)
...
test('保存接待方案时主模型失效会被阻止且不生成版本', function () {
    $provider = createReceptionTestProvider();
    $model = createReceptionTestModel($provider, ['is_active' => false]);
    $task = createReceptionTestModel($provider, [
        'name' => 'Task Model',
        'model_id' => 'task-model',
        'sort_order' => 1,
...
test('非所有者无法访问接待方案管理页', function () {
    ...
    $plan = ReceptionPlan::factory()->create(['workspace_id' => $this->workspace->id]);
...
test('所有者无法访问其他工作区的方案详情', function () {
    $otherWorkspace = Workspace::factory()->create();
    $this->user->workspaces()->attach($otherWorkspace, ['role' => 'owner']);

    $localPlan = ReceptionPlan::factory()->create(['workspace_id' => $this->workspace->id]);
    $otherPlan = ReceptionPlan::factory()->create(['workspace_id' => $otherWorkspace->id]);
    ...
    $this->actingAs($this->user)
        ->delete(route('workspace.manage.reception.plans.destroy', ['slug' => $this->workspaceSlug(), 'plan' => $otherPlan->id]))
        ->assertNotFound();
});

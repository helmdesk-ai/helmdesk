...
test('所有者可以将 .md 文件上传到知识库默认分组下', function () {
    ...
    expect($document->workspace_id)->toBe((string) $this->workspace->id)
...
test('问答知识库不能上传普通文档', function () {
    ...
    $this->actingAs($this->user)
        ...
        ->post(
            route('workspace.manage.knowledge-bases.documents.store', [
                'slug' => $this->workspaceSlug(),
                'knowledgeBase' => $qaKnowledgeBase->id,
            ]),
            ['files' => [$file]]
        )
...
test('其他工作区用户无法读取当前知识库文档原文件', function () {
    Storage::fake('local');

    $file = UploadedFile::fake()->createWithContent('guide.md', '# Guide');
    ...
    $document = KnowledgeDocument::query()->firstOrFail();
    $outsiderWorkspaceSlug = $this->workspaceSlug();
    $outsider = $this->createUserWithWorkspace();

    $this->actingAs($outsider)
        ->get(
            route('workspace.manage.knowledge-bases.documents.preview-file', [
                'slug' => $outsiderWorkspaceSlug,
                'knowledgeBase' => $this->kb->id,
                'document' => $document->id,
            ])
        )
...
test('其他工作区用户无法上传到当前知识库', function () {
    $outsiderWorkspaceSlug = $this->workspaceSlug();
    $outsider = $this->createUserWithWorkspace();
...
test('其他工作区用户无法删除当前知识库文档', function () {
    $outsiderWorkspaceSlug = $this->workspaceSlug();
    $outsider = $this->createUserWithWorkspace();
    ...
    expect(KnowledgeDocument::query()->whereKey($document->id)->exists())->toBeTrue();
});
...
test('所有者可以将文档移动到另一个分组', function () {
    ...
    $this->actingAs($this->user)
        ->put(
            route('workspace.manage.knowledge-bases.documents.move', [
                'slug' => $this->workspaceSlug(),
                'knowledgeBase' => $this->kb->id,
                'document' => $document->id,
            ]),
...
test('其它工作区用户无法在当前知识库手动添加内容', function () {
    $outsiderWorkspaceSlug = $this->workspaceSlug();
    $outsider = $this->createUserWithWorkspace();
...
test('其它工作区用户无法编辑当前知识库的手动文档', function () {
    $outsiderWorkspaceSlug = $this->workspaceSlug();
    $outsider = $this->createUserWithWorkspace();

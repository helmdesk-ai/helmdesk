...
function systemTranslationProvider(string $slug = 'google-tr'): TranslationProvider
{
...
test('未登录用户被重定向到登录页', function () {
    $this->get(route('admin.manage.translation.providers.index'))
...
test('超级管理员可以查看翻译供应商列表页', function () {
    ...
    $this->actingAs($this->user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ->component('systemSettings/translationProviders/Index')
            ->has('providers', 1)
...
test('创建新的翻译供应商后保持待配置状态', function () {
    $this->actingAs($this->user)
        ->post(route('admin.manage.translation.providers.store'), [
            'name' => 'My Google',
...
test('新建表单可以用未保存的凭据测试翻译供应商', function () {
    ...
    $this->actingAs($this->user)
        ->postJson(route('admin.manage.translation.providers.check-new'), [
            'text' => 'Hello',
            'target_lang' => 'zh-CN',
            'protocol' => TranslationProviderType::GoogleTranslate->value,
...
test('更新凭据时 secret 字段提交空值会保留原值', function () {
    $provider = systemTranslationProvider();

    $this->actingAs($this->user)
        ->put(route('admin.manage.translation.providers.update', ['provider' => $provider->slug,
        ]), [
            'name' => $provider->name,
...
test('清空凭据后供应商不再具备完整凭据', function () {
    ...
    $this->actingAs($this->user)
        ->delete(route('admin.manage.translation.providers.clear-credentials', ['provider' => $provider->slug,
        ]))
        ->assertRedirect();
...
test('测试翻译连通 - 成功', function () {
    ...
    $this->actingAs($this->user)
        ->postJson(route('admin.manage.translation.providers.check', ['provider' => $provider->slug,
        ]), [
            'text' => 'Hello',
            'target_lang' => 'zh-CN',
            'source_lang' => 'en',

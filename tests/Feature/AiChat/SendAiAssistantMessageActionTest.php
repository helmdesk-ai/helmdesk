...
test('它拒绝过大的聊天历史来自工作区路由', function () {
    ...
    $this->actingAs($user)
        ->postJson(route('workspace.ai-chat.messages.store', ['slug' => $workspace->slug]), [
            'prompt' => 'hello',
            'model_id' => $model->id,
...
test('它转发已启用的 MCP 服务和工具白名单到 Go 桥接', function () {
    ...
    $inactiveServer = McpServer::factory()->for($workspace)->create([
        'is_active' => false,
    ...
    $emptyServer = McpServer::factory()->for($workspace)->create([
        'is_active' => true,
...
test('它把工作区下的知识库列表与 workspace_id 一起下发给 Go 桥接', function () {
    $workspace = Workspace::factory()->create();
    $provider = createAiChatTestProvider($workspace);
    $model = createAiChatTestModel($provider);
    ...
    Http::assertSent(function ($request) use ($workspace, $kb): bool {
        if (($request['workspace_id'] ?? null) !== (string) $workspace->id) {
            return false;
        }
...
test('它把 MCP 空凭据和空请求头序列化为 JSON 对象', function () {
    ...
    $server = McpServer::factory()->for($workspace)->create([
        'is_active' => true,

...
function buildWebChannel(SystemContext $systemContext, array $mappings): Channel
{
...
function freshContact(SystemContext $systemContext): Contact
{
...
test('未配置映射或空 query 直接 noop', function () {
    ...
    $contact = freshContact($this->systemContext);
...
test('OnlyIfEmpty + 联系人未填名时写入', function () {
    $channel = buildWebChannel($this->systemContext, [
        [
            'param_name' => 'name',
            'target' => 'contact_name',
            'trust' => 'always',

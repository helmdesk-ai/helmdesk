...
test('单租户后台不再保留系统常规设置页面', function (): void {
    $generalSetting = File::get(resource_path('js/pages/admin/generalSetting/Index.vue'));

    expect(File::exists(resource_path('js/pages/currentSystem/Index.vue')))->toBeFalse()
        ->and($generalSetting)->not->toContain("t('系统ID不可修改')");
});

test('系统模型表单移除输入类型配置和容量字段', function (): void {
...
test('大模型供应商页面不再展示模型输入类型标签', function (): void {
    $contents = File::get(resource_path('js/pages/systemSettings/aiProviders/Index.vue'));
...
test('原系统设置菜单已迁入总管理后台和系统设置二级视图', function (): void {

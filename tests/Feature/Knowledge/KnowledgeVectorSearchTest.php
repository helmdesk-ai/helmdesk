...
beforeEach(function (): void {
    ...
    $this->provider = makeUsableAiProvider();
    $this->embeddingModel = makeAiModel(AiModelPurpose::Embedding, $this->provider);
...
test('VectorRetriever 跨 systemContext 时仍能从小集合的目标 systemContext 召回到节点', function (): void {
    ...
    $otherModel = makeAiModel(AiModelPurpose::Embedding);
...
test('VectorRetriever 按 embedding_model_id 隔离：切换模型后不同模型向量不会参与召回', function (): void {
    ...
    $newModel = makeAiModel(AiModelPurpose::Embedding, $this->provider);

...
use App\Actions\KnowledgeBase\Indexing\RebuildSystemKnowledgeIndexAction;
 ...
 * 检索配置变更后重建索引的批量任务。
 *
 * 把"保存设置 → 遍历全部文档/问答 → 清理索引 → 派发新的逐条 Job"这条耗时链路从 HTTP 请求里剥出来，
 * 避免后台几千条文档导致接口超时；UpdateSystemKnowledgeSettingsAction 只负责派发本 Job。
 *
 * `resetVectorTables=true` 是维度变更的快速路径：vec0 虚表整体 drop，所有策略的节点也跟着清空，
 * 后续逐条索引 Job 会按需重新建表与写节点，避免在维度变化后做无意义的逐节点 vec0 清理。
 */
class RebuildSystemKnowledgeIndexJob implements ShouldQueue
{
    use Queueable;
     ...
     * 重建系统下文档和 QA 的索引。
     */
    public function handle(RebuildSystemKnowledgeIndexAction $action): void
    {
        $action->handle(

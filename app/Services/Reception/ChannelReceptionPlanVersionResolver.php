...
class ChannelReceptionPlanVersionResolver
{
     ...
     */
    public function resolveChannelStatus(SystemContext $systemContext, Channel $channel): ?ModelSelectionStatusData
    {
        ...
        return $this->resolveVersionStatus($systemContext, $version);
    }

    /**
     * 直接对版本对象做可用性解析；用于版本列表 / 选项渲染等需要复用同一规则的场景。
     */
    public function resolveVersionStatus(SystemContext $systemContext, ReceptionPlanVersion $version): ModelSelectionStatusData
    {
        ...
        $modelStatus = $this->resolver->resolveModelStatus($systemContext, $modelId);

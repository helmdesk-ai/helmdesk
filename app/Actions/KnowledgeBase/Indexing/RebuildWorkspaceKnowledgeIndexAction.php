...
class RebuildWorkspaceKnowledgeIndexAction
{
    ...
    public function handle(
        ...
        foreach ($strategies as $strategy) {
            $this->rebuildDocumentStrategy($workspace, $strategy, $resetVectorTables);
        }

        if ($rebuildQaVectorIndex) {
            $this->rebuildQaVectorIndex($workspace, $resetVectorTables);
        }
    }
    ...
    private function rebuildDocumentStrategy(Workspace $workspace, KnowledgeIndexingStrategy $strategy, bool $resetVectorTables): void
    {
        KnowledgeDocument::query()
            ...
            ->each(function (KnowledgeDocument $document) use ($workspace, $strategy, $resetVectorTables): void {
                ...
                $knowledgeBase->setRelation('workspace', $workspace);

                if (! $resetVectorTables) {
                    $this->nodes->purgeStrategyForDocument($document, $strategy);
                }

                $status = $workspace->hasKnowledgeIndexingStrategy($strategy)
                ...
                if ($document->parse_status === KnowledgeDocumentParseStatus::Succeeded
                    && $workspace->hasKnowledgeIndexingStrategy($strategy)) {
                    match ($strategy) {
     ...
     * 遍历问答条目，清向量索引并按需重新派发。
     */
    private function rebuildQaVectorIndex(Workspace $workspace, bool $resetVectorTables): void
    {
